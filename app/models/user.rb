class User < ActiveRecord::Base
  ...
  def eligible_for_new_match?
    if self.number_of_matches == 2
      ...
      if self.current_match
        (Time.now - self.current_match.created_at) > 518400 #six days
      else
        true
      end
    end
  ...
  def remove_first_degree_connections(users)
    ...
    users.reject do |user|
      if user.eligible_for_new_match?
        self.connections.pluck(:linkedin_user_id).include?(user.linkedin_user_id) || user.connections.pluck(:linkedin_user_id).include?(self.linkedin_user_id)
      else
        true
      end
    end
  ...
  def self.get_connections(user)
    ...
    user_connections = client.connections
    if user_connections["total"] != 0
      user.total_connections = user_connections["total"]
      user.save!
      user_connections["all"].each do |connection|
